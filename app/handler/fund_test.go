...
func TestFundHandler(t *testing.T) {
	...
	investStatusMock := &InvestStatusIndicatorMock{}
	f := NewFundHandler(readerMock, writerMock, investMock, exGetterMock, investStatusMock)
	f.InitRoute(app)
	...
	t.Run("AvailableAmounts 테스트", func(t *testing.T) {
		t.Run("양수 가용 금액 테스트", func(t *testing.T) {
			// Setup: 매수 가능한 금액 반환
			*investStatusMock = InvestStatusIndicatorMock{availableAmount: 50000.0, err: nil}

			var resp float64
			err := sendReqeust(app, "/funds/1/available_amounts", "GET", nil, &resp)

			assert.NoError(t, err)
			assert.Equal(t, 50000.0, resp)
			t.Logf("Available amount: %.2f (positive means can buy)", resp)
		})

		t.Run("음수 가용 금액 테스트", func(t *testing.T) {
			// Setup: 매도 필요한 상황 (음수 반환)
			*investStatusMock = InvestStatusIndicatorMock{availableAmount: -25000.0, err: nil}

			var resp float64
			err := sendReqeust(app, "/funds/1/available_amounts", "GET", nil, &resp)

			assert.NoError(t, err)
			assert.Equal(t, -25000.0, resp)
			t.Logf("Available amount: %.2f (negative means need to sell)", resp)
		})

		t.Run("0 가용 금액 테스트", func(t *testing.T) {
			// Setup: 현재 포트폴리오가 이상적인 상태
			*investStatusMock = InvestStatusIndicatorMock{availableAmount: 0.0, err: nil}
			...
			t.Logf("Available amount: %.2f (zero means optimal portfolio)", resp)
		})
		...
		t.Run("InvestStatusIndicator 오류 테스트", func(t *testing.T) {
			// InvestAvailableAmount 오류 시뮬레이션
			*investStatusMock = InvestStatusIndicatorMock{availableAmount: 0, err: fmt.Errorf("investment calculation error")}

			var resp interface{}
			err := sendReqeust(app, "/funds/1/available_amounts", "GET", nil, &resp)

			assert.Error(t, err)
			*investStatusMock = InvestStatusIndicatorMock{availableAmount: 0, err: nil} // Reset for other tests
			t.Logf("Expected error for investment calculation error: %v", err)
		})
	})
